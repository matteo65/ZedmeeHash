...
static uint64_t default_table[256];
 ...
 **/
uint64_t zedmeehash64(const char *data, int pos, size_t length, uint64_t seed, uint64_t table[]) 
{
	data += pos;
	while(length)
 ...
 **/
uint64_t zedmeehash64def(const char *data, int pos, size_t length, uint64_t seed)
{
	data += pos;
	while(length)
